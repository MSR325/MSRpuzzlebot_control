...
class LineFollowerCentroid(Node):
    ...
    def image_callback(self, msg):
        ...
        if line_m:
            ...
            recent_avg = np.mean(self.middle_line_buffer[-3:]) if len(self.middle_line_buffer) >= 3 else current_x
            x_diff = abs(current_x - recent_avg)

            self.get_logger().info(f"[🟢 LINE M DETECTED] x={current_x:.1f}, avg3={recent_avg:.1f}, x_diff={x_diff:.1f}")

            if x_diff < self.x_diff_threshold:
                self.last_valid_middle_x = current_x
                self.drift_frame_count = 0
                use_middle_line = True
                self.get_logger().info("[✅ MIDDLE OK] x_diff within threshold — using current middle line.")
            else:
                self.drift_frame_count += 1
                if self.drift_frame_count >= self.max_drift_frames:
                    self.get_logger().warn("⚠️ Middle line lost! Switching to nearest side line.")
                    use_middle_line = False
                else:
                    self.get_logger().info("[🔄 JITTER SUPPRESSED]. Holding last known good position.")
            ...
            if use_middle_line:
                ...
                self.get_logger().info(f"[🧭 FOLLOWING MIDDLE] x_error={x_error:.1f}, angle_error={angle_error:.1f}, linear_x={linear_speed:.2f}, angular_z={angular_speed:.2f}")
                # self.get_logger().info(f"Publishing: angle_error={angle_error:.3f}, x_error={x_error:.3f}, linear_x={linear_speed:.3f}, angular_z={angular_speed:.3f}")
            else:
                self.get_logger().info("[🧭 FALLBACK MODE] Trying to find closest side line to last known middle.")
                if self.last_valid_middle_x is not None:
                # Choose side line closest to previous middle
                    twist = Twist()
                    candidates = []
                    if line_l:
                    ...
                    if candidates:
                        chosen_label, chosen_line, _ = min(candidates, key=lambda x: x[2])
                        x_error = chosen_line["x_global"] - frame_center_x
                        angle_error = 90.0 - chosen_line["angle"] if chosen_line["angle"] > 0 else -90.0 - chosen_line["angle"]
                        if abs(angle_error) < self.ang_e_thrsh:
                            angle_error = 0.0
                        ...
                        self.get_logger().info(
                            f"[🔁 USING {chosen_label}] x_error={x_error:.1f}, angle_error={angle_error:.1f}, linear_x={twist.linear.x:.2f}, angular_z={twist.angular.z:.2f}"
                        )

                    else: 
                        # Fallback behavior when no center line is detected
                        twist.linear.x = -0.15 * self.color_flag_multiplier
                        twist.angular.z = 0.0
                        self.publisher.publish(twist)
