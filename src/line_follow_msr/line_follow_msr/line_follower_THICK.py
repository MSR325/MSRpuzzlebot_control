...
class LineFollowerCentroid(Node):
    ...
    def timer_callback(self):
        ...
        if self.homography_matrix is not None:
            ...
            # Warp with secondary homography and show it
            if self.secondary_homography is not None:
                ...
                # Preprocess
                binary_l2 = self.preprocess_region(roi_l2, blur_k, block_size, c_bias, morph_k)
                binary_m2 = self.preprocess_region(roi_m2, blur_k, block_size, c_bias, morph_k)
                binary_r2 = self.preprocess_region(roi_r2, blur_k, block_size, c_bias, morph_k)

                # Detect lines
                _ = self.detect_line_in_roi(binary_l2, 0, overlay2, (0, 0, 255), "L2", min_area)
                _ = self.detect_line_in_roi(binary_m2, 2 * twelve_div2, overlay2, (0, 255, 255), "M2", min_area)
                _ = self.detect_line_in_roi(binary_r2, 7 * twelve_div2, overlay2, (0, 0, 255), "R2", min_area)

                # Display overlay2
                overlay2_resized = cv2.resize(overlay2, None, fx=3.0, fy=3.0)
                cv2.imshow("Overlay Secondary", overlay2_resized)
        ...
        if chosen_line is not None:
            ...
            # -------- curvature bonus only if middle line is chosen --------
            if chosen_label == 'M' and self.detected_mid_radius < self.mid_radius_thrshld:
                radius_term   = self.radius_kp * self.detected_mid_radius
                angular_speed = angular_speed - radius_term   # same sign convention as original node
            ...
            self.get_logger().info( f"[{chosen_label}] x_err={x_error:.1f}, ang_err={angle_error:.1f}, "f"R={self.detected_mid_radius:.1f}, ω={angular_speed:.3f}")
        else:
