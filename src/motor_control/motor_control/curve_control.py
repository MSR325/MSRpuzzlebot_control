#!/usr/bin/env python3
"""
ik_path_follower.py  –  Pure-Pursuit + ramp-limiter
 junio 2025
"""
...
class PathFollower(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('a_max',          0.50)   # m/s²
        self.declare_parameter('alpha_max',      4.00)   # rad/s²
        ...
        self.last_twist     = Twist()                   # para rampa
        ...
        self.get_logger().info('🚀 ik_path_follower listo (100 Hz).')
    ...
    def _sync_params(self):
        ...
        self.a_max       = p('a_max').value
        self.alpha_max   = p('alpha_max').value
    ...
    def control_loop(self):
        ...
        while self.current_path and \
              self._dist_xy(self.current_path[0].pose.position, x, y) < self.lookahead/2:
        ...
        # 6) Aplicar ramp-limiter en Twist
        now  = self.get_clock().now()
        dt   = (now - self.last_stamp).nanoseconds * 1e-9
        self.last_stamp = now

        v_prev = self.last_twist.linear.x
        w_prev = self.last_twist.angular.z

        dv = max(-self.a_max*dt, min(self.a_max*dt, v_cmd - v_prev))
        dw = max(-self.alpha_max*dt, min(self.alpha_max*dt, w_cmd - w_prev))

        twist = Twist()
        twist.linear.x  = v_prev + dv
        twist.angular.z = w_prev + dw
        ...
        if self._dist_xy(last_wp, x, y) < self.arrival_tol:
            self.current_path.clear()  # se vacía → próximo loop publicará stop
