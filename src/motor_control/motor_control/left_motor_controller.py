...
class LeftMotorController(Node):
    def __init__(self):
        super().__init__('left_motor_controller')
        ...
        # ROS Interfaces
        self.create_subscription(Float32, '/left/set_point', self.set_point_callback, 10)
        self.create_subscription(Float32, '/VelocityEncL', self.feedback_callback, qos_profile)
        self.cmd_pub = self.create_publisher(Float32, '/ControlL', 10)
        self.filtered_pub = self.create_publisher(Float32, '/VelocityEncL_filtered', 10)

        self.timer = self.create_timer(self.sample_time, self.control_loop)
        self.add_on_set_parameters_callback(self.parameter_callback)

        self.get_logger().info('Left Motor Controller with filter and debug topic initialized')
